...
try
{
    ...
    {
        SPDLOG_ERROR("Caught exception while trying to draw node '{0}': {1}", std::string(NodeID), e.what());
    }
...
SimpleNodeView::SimpleNodeView(const flow::SharedNode& node) : NodeView(node)
{
...
try
{
    ...
    try
    {
        ...
        const ImVec2 text_size = ImGui::CalcTextSize(Name.c_str()) * 2.f;
    ...
    draw_list->AddText(std::bit_cast<ImFont*>(GetConfig().NodeHeaderFont.get()), 40.f,
                       ...
                       IM_COL32(180, 180, 180, 255), Name.c_str());
...
void CommentView::Draw()
{
    ...
    if (_edit)
    {
        ...
        if (ImGui::InputText("", &Name, ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_EnterReturnsTrue))
        {
            _edit = false;
        }
        ImGui::PopItemWidth();
